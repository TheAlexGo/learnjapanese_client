...
const Dicts = () => {
  ...
  useEffect(() => {
    if(isLoadContent) {
      fetch(`/api/v0/dicts/${id_user}/${id}/words`).then(r => r.json())
        .then(data => dispatch(fillWord(id, data.data)));
      setIsLoadContent(false);
    }
  }, [dispatch, id, id_user, isLoadContent])

  const content = <CardWords id_dict={id} words={data} callback={setIsLoadContent} />;
